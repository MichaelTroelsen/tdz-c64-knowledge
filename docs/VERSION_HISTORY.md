...
Detailed per-release notes for the TDZ C64 Knowledge Base.

> **Provenance:** this content previously lived as a ~73KB `VERSION_HISTORY`
> string literal inside `version.py`, where it made that module 1871 lines
> long and was parsed on every single startup despite no code ever reading it.
> It is reproduced here verbatim.
>
> Note that this file and [CHANGELOG.md](CHANGELOG.md) grew independently and
> overlap: this one carries detailed entries for 2.23.19-2.23.36 that
> CHANGELOG.md never received, while CHANGELOG.md has its own formatting and
> some entries this lacks. They have not been merged, because reconciling them
> would mean rewriting release history that cannot be verified after the fact.
> Treat this as the more complete record for 2.23.19 onward.
